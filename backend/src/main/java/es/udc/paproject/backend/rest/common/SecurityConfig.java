...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.cors().and().csrf().disable()
			...
			// CATALOG SERVICE
			.antMatchers(HttpMethod.POST, "/catalog/add").hasRole("USER")
			.antMatchers(HttpMethod.GET, "/catalog/categories").permitAll()
			.antMatchers(HttpMethod.GET, "/catalog/products/*").permitAll()
			.antMatchers(HttpMethod.GET, "/catalog/products").permitAll()
			// BID SERVICE
			.antMatchers(HttpMethod.GET, "/bids/user").hasRole("USER")
			.antMatchers(HttpMethod.POST, "/bids/products/*").hasRole("USER")
			.anyRequest().denyAll();
